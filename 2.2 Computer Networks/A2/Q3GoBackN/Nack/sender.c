...
int waitForACKs(int sockfd, int *base, int ws)
{
    ...
    if (activity > 0)
    {
        ...
        while ((val = recv(sockfd, buffer, FRAME_SIZE, MSG_DONTWAIT)) > 0)
        {
            ...
            else if (sscanf(buffer, "<NACK>%d</NACK>", &nack_seq) == 1)
            {
                printf("\n[RECV] Received NACK %d. Resending window from Frame %d\n", nack_seq, *base);
                return -1; // signal that a resend is needed immediately
            }
        }
        return 1;
    }
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        int ack_result = waitForACKs(sockfd, &base, window_size);
        ...
        { // NACK received or timeout
            printf("\n\n[RESENDING] Resending window from Frame %d\n", base);
