// Receiver.c - Selective Repeat ARQ
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
void sendACK(int sockfd, int seq)
{
    ...
    // Wait a bit to allow ACK to be delivered
    usleep(10000);  // 10ms
}
        ...
        if (sscanf(buffer, "<SEQ>%d</SEQ><DATA>%[^<]</DATA><CHK>%d</CHK>", &seq, data, &recv_chk) == 3)
        {
            ...
            if (seq >= base && seq < base + window_size)
            {
                ...
                if (chk == recv_chk)
                {
                    ...
                    // Always send ACK in Selective Repeat, even for duplicates
                    sendACK(newsockfd, seq);
                }
                else
                {
                    printf("[REJECTED] Frame %d has invalid checksum\n", seq);
                    // No ACK sent for corrupt frames
                }
            }
            else if (seq < base)
            {
                ...
                // In Selective Repeat, we still send ACK for duplicate frames
                sendACK(newsockfd, seq);
            }
            else
            {
                ...
                // No ACK sent for out-of-window frames
            }
        }
    }
